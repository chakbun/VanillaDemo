...
if [ ! -n "$1" ] ;then
echo "--- ❌ 分支名没有输入，需要重传 ----"
echo "示例: auto_proto_platform_plugin.sh develop"
exit
else
echo "--- 分支名为: ${Branch}  ----"
fi
...
#仓库地址
RepoPath=ssh://git@git.2tianxin.com:2022/platform/protocol/plugin.git
#输出工程的oc文件路径
...
#3. 移动需要的文件
echo "移动 plugin_pb 文件"
mkdir plugin_pb
#记得确定一下仓库存放plugin_pb文件路径
mv  ./protoRepo/plugin_pb/* ./plugin_pb
echo "移动完成"
