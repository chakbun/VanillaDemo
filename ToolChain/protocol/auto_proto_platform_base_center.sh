...
#仓库地址
RepoPath=ssh://git@git.2tianxin.com:2022/platform/protocol/base_center.git
#输出工程的oc文件路径
...
echo "移动 base_pb 文件"
mkdir base_pb
#记得确定一下仓库存放plugin_pb文件路径
mv  ./protoRepo/base_pb/* ./base_pb
echo "移动完成"
...
#platform
rm -rf GEN_DIR/base_pb
for file in `ls ./base_pb`
do
#.ext.proto
if [[ `echo $file | awk -F'.' '$0~/.*ext.*proto/{print $3}'` = "proto" ]]
then
echo "name: ${file} ${GEN_DIR}/base_pb"
