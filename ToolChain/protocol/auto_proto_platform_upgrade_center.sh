...
#3. 移动需要的文件
echo "移动 upgrade_center_pb 文件"
mkdir upgrade_center_pb
#记得确定一下仓库存放upgrade_center_pb文件路径
mv  ./protoRepo/* ./upgrade_center_pb
echo "移动完成"
...
#platform
rm -rf GEN_DIR/upgrade_center_pb
for file in `ls ./upgrade_center_pb`
do
#.ext.proto
if [[ `echo $file | awk -F'.' '$0~/.*ext.*proto/{print $3}'` = "proto" ]]
then
echo "name: ${file} ${GEN_DIR}/upgrade_center_pb"
./protoc --objc_out=${GEN_DIR} \
upgrade_center_pb/$file
fi
done
